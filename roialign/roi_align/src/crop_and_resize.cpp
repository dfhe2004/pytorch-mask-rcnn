...
extern "C" void CropAndResizePerBox(
...
extern "C" void crop_and_resize_forward(
    THFloatTensor * image,
    ...
    const int batch_size = THFloatTensor_size(image,0); //image->size[0];
    const int depth = THFloatTensor_size(image,1); //image->size[1];
    const int image_height = THFloatTensor_size(image,2); //image->size[2];
    const int image_width = THFloatTensor_size(image,3); //image->size[3];

    const int num_boxes = THFloatTensor_size(boxes,0); //boxes->size[0];
...
extern "C" void crop_and_resize_backward(
    THFloatTensor * grads,
    THFloatTensor * boxes,      // [y1, x1, y2, x2]
    THIntTensor * box_index,    // range in [0, batch_size)
...
{   
    // shape
    const int batch_size = THFloatTensor_size(grads_image,0); // grads_image->size[0];
    const int depth = THFloatTensor_size(grads_image,1); //grads_image->size[1];
    const int image_height = THFloatTensor_size(grads_image,2); //grads_image->size[2];
    const int image_width = THFloatTensor_size(grads_image,3); //grads_image->size[3];

    const int num_boxes = THFloatTensor_size(grads,0); //grads->size[0];
    const int crop_height = THFloatTensor_size(grads,2); //grads->size[2];
    const int crop_width = THFloatTensor_size(grads,3); //grads->size[3];
