...
extern "C"  int cpu_nms(THLongTensor * keep_out, THLongTensor * num_out, THFloatTensor * boxes, THLongTensor * order, THFloatTensor * areas, float nms_overlap_thresh) {
    ...
    // Number of ROIs
    int64_t boxes_num = THFloatTensor_size(boxes, 0);
    int64_t boxes_dim = THFloatTensor_size(boxes, 1);
